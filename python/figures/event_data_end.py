...
pd.to_pickle(snakemake, '.event_data_end.py.pkl')
# snakemake = pd.read_pickle('.event_data_end.py.pkl')
...
for sid in samples['SID']:
    ...
    movement_events = pd.read_csv(f'{prefix}/movement_end_final.csv')
    ...
    for index, event in movement_events.iterrows():
        emg_data = data.copy().crop(tmin = event['End'] - 0.4, tmax = event['End'] + 0.4)
        x = emg_data.times
        emg_y = emg_data.get_data(picks = [event['Channel']])[0]

        tke_data = tke.copy().crop(tmin = event['End'] - 0.4, tmax = event['End'] + 0.4)
        tke_y = tke_data.get_data(picks = [event['Channel']])[0]

        vm_data = vm.copy().crop(tmin = event['End'] - 0.4, tmax = event['End'] + 0.4)
