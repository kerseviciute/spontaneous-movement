...
pd.to_pickle(snakemake, '.ap_time_end.py.pkl')
# snakemake = pd.read_pickle('.ap_time_end.py.pkl')
...
for index, sample in samples.iterrows():
    ...
    for i, event in movement_events.iterrows():
        event_data = data.copy().pick(picks = event['ChannelID']).crop(tmin = event['End'] - 0.4, tmax = event['End'] + 0.4)
