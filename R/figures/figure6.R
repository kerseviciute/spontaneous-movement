...
event_data <- event_data %>%
  .[ , Time := Time - unique(EventEnd) ] %>%
  .[ , EventEnd := 0 ] %>%
  .[ , Type := 'None' ] %>%
  .[ Time >= -0.4 & Time <= -0.2, Type := 'L' ] %>%
  .[ Time >= -0.1 & Time <= 0, Type := 'O' ] %>%
  .[ Time > 0 & Time <= 0.2, Type := 'P' ] %>%
  .[ Time > 0.2 & Time <= 0.4, Type := 'B' ] %>%
...
type_average <- event_data %>%
  ...
  .[ Type == 'P', Type := 'Post-movement' ] %>%
  .[ Type == 'O', Type := 'Movement offset' ] %>%
...
change <- foreach(sid = type_average[ , unique(SID) ], .combine = rbind) %do% {
  ...
  data.table(
    SID = sid,
    Type = c('Post-movement', 'Movement offset', 'Late movement'),
    Change = c(postmovement - baseline, offset - baseline, late - baseline),
    Region = type_average[ SID == sid, unique(Region) ]
  )
} %>%
  .[ , Type := factor(Type, levels = c('Baseline', 'Post-movement', 'Movement offset', 'Late movement')) ]
